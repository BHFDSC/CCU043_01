...
# MAGIC %md # CCU043_01-D08a-final_cohort
# MAGIC  
# MAGIC **Description** This notebook creates the final cohort and survival dataset with covariates, outcomes, and follow up times.
# MAGIC  
...
# MAGIC
# MAGIC **Data Output**
# MAGIC - **`ccu043_01_final_cohort_all_data`** : final cohort with exposed non-exposed individuals, convariates, and outcomes
# MAGIC - **`ccu043_01_survival_cohort`** : final dataset for survival analysis 
...
cohort = (cohort
          ...
          .withColumn('outcome_t1',
                  f.when((f.col('covid_censor_date').isNull()) 
                          & (f.col('diab_name') == 'diabetes_type1'),
                              f.lit(1)
                              )
                  .when((f.col('covid_censor_date').isNotNull()) 
                          & (f.col('diab_name') == 'diabetes_type1')
                          & (f.col('diab_date') <= f.col('covid_censor_date')),
                              f.lit(1)
                              )
                  .otherwise(f.lit(0))
                  )
          .withColumn("followup_end_t1",
                      f.when(f.col("outcome_t1") == 1, f.col("diab_date"))
          ...
          .withColumn('outcome_t2',
                  f.when((f.col('covid_censor_date').isNull()) 
                          & (f.col('diab_name') == 'diabetes_type2'),
                              f.lit(1)
                              )
                  .when((f.col('covid_censor_date').isNotNull()) 
                          & (f.col('diab_name') == 'diabetes_type2')
          ...
          .withColumn("followup_end_t2",
                      f.when(f.col("outcome_t2") == 1, f.col("diab_date"))
                      .otherwise(f.least(f.col("date_of_death"), f.col("study_end_date"), f.col("covid_censor_date"))) 
                      )
...
tmpt = tab(cohort, 'diab_name', var2_unstyled=1); print()
...
save_table(df=cohort, out_name=f'{proj}_final_cohort_all_data', save_previous=False)
cohort = spark.table(f'{dsa}.{proj}_final_cohort_all_data')
count_varlist(cohort, ['person_id', 'index_date'])
...
surv_cohort = (cohort
               .select('person_id', 'index_date', 'exposed',
                        'age', 'sex', 'ethnicity', 'region', 'imd', 'bmi_value',
...
save_table(df=surv_cohort, out_name=f'{proj}_survival_cohort', save_previous=False)
surv_cohort = spark.table(f'{dsa}.{proj}_survival_cohort')
...
df = cohort.select('person_id','index_date', 'exposed', 'age',
'sex','ethnicity','region','imd',
'date_of_death','outcome_t1','followup_end_t1','followup_time_t1','outcome_t2','followup_end_t2','followup_time_t2','bmi_value',
)
...
df_bmi = (cohort.select('person_id', 'exposed', 'sex',"age",'ethnicity', 'imd', 'region','smoking_cat',
                   'outcome_t1','followup_end_t1','followup_time_t1','outcome_t2','followup_end_t2','followup_time_t2','bmi_cat','bmi_value',)
          .filter(f.col('bmi_value').isNotNull())
)
df_bmi = df_bmi.select('*').withColumn(
    "age_band",
    f.when(f.col("age").between(18, 29), "18-29")
    .when(f.col("age").between(30, 39), "30-39")
    .when(f.col("age").between(40, 49), "40-49")
...
# MAGIC %md
# MAGIC ## 5.1 Baseline Table

# COMMAND ----------

categorical_vars_dict = {
    'age_band':'Age (years), n (%)',
    'ethnicity':'Ethnicity, n (%)',
    'imd':'IMD (quintiles), n (%)',
    'region':'Region, n (%)',
    'bmi_cat':'BMI (Kg/m2), n (%)',
...
for cat_var in categorical_vars:
    for exposed_value in [0, 1]:
        for sex in ['All', 'M', 'F']:
            if sex == 'All':
                df_filtered = df_bmi.filter(f.col("exposed") == exposed_value)
            else:
                df_filtered = df_bmi.filter(f.col("exposed") == exposed_value).filter(f.col("sex") == sex)
            window_spec = Window.partitionBy()
            summary = (
                df_filtered
                .groupBy(cat_var)
...
i = 0
for exposed_value in [0, 1]:
    t = df_bmi.filter(f.col("exposed") == exposed_value).select(f.col('person_id')).count()
    for sex in ['All', 'M', 'F']:
        if sex == 'All':
            df_filtered = df_bmi.filter(f.col("exposed") == exposed_value)
        else:
...
summary = cat_summary.unionByName(cont_summary.drop('order'))
display(summary)

# COMMAND ----------

# MAGIC %md
# MAGIC ## 5.2 Incidence Rate
